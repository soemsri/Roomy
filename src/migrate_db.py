...
def migrate():
    ...
    commands = [
        ...
        "CREATE TABLE IF NOT EXISTS booking_requests (id INTEGER PRIMARY KEY AUTOINCREMENT, uuid TEXT UNIQUE NOT NULL, line_user_id TEXT NOT NULL, full_name TEXT NOT NULL, phone_number TEXT NOT NULL, workplace_name TEXT NOT NULL, job_position TEXT NOT NULL, workplace_phone TEXT NOT NULL, requested_move_in_date DATETIME NOT NULL, preferred_building_id INTEGER REFERENCES buildings(id), preferred_room_id INTEGER REFERENCES rooms(id), assigned_room_id INTEGER REFERENCES rooms(id), needs_bed INTEGER DEFAULT 0, needs_mattress INTEGER DEFAULT 0, agreement_accepted INTEGER DEFAULT 1, agreement_accepted_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP, status TEXT DEFAULT 'Pending', admin_notes TEXT, language TEXT DEFAULT 'th', created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP, updated_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP)",
        "ALTER TABLE booking_requests ADD COLUMN needs_bed INTEGER DEFAULT 0",
        "ALTER TABLE booking_requests ADD COLUMN needs_mattress INTEGER DEFAULT 0",
        ...
        # New: parcels table for Parcel Management System
        "CREATE TABLE IF NOT EXISTS parcels (id INTEGER PRIMARY KEY AUTOINCREMENT, room_id INTEGER NOT NULL, tenant_id INTEGER, carrier TEXT NOT NULL, tracking_number TEXT, parcel_image_url TEXT, status TEXT DEFAULT 'pending', arrived_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP, received_at TIMESTAMP, received_by_name TEXT, proof_image_url TEXT, notes TEXT, created_by_user_id INTEGER REFERENCES users(id), created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP, updated_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP, FOREIGN KEY (room_id) REFERENCES rooms(id), FOREIGN KEY (tenant_id) REFERENCES tenants(id), FOREIGN KEY (created_by_user_id) REFERENCES users(id))",
